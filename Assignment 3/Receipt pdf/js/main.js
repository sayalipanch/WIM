...
$(document).ready(function () {
    ...
    var createFormDataObject = function () {
        ...
        if (info.billNo == '') {
            var now = new Date();
            info.billNo = '' + now.getFullYear() + (now.getMonth() + 1) + now.getDate() + now.getTime();
        }
    ...
    if (devMode) {
        ...
        $('input#billNo').val('2341');
        $('input#purchaser').val("Eren Yeager");
        $('textarea#purchaser-address').val("B-221, Ram Nagar\nMatunga\nMaharashtra, India");
        $('input#purchaser-tin').val("THSPD1251Y");

        $('.items .item input.item-name').val("Harry Potter");
